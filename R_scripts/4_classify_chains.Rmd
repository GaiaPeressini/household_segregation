...
first_move_byinit <- vcs[chain_step == 1] %>% 
  group_by(initiation) %>%
  count(seg_move) %>%
   mutate(Move = ifelse(seg_move == -1, "Desegregating", ifelse(seg_move == 1, "Segregating", "Within")),
         Initiation = ifelse(initiation == "deceased", "Deceased", 
                             ifelse(initiation == "new_apartment", "New apartment", "Out move"))) %>%
  rename(Count = n) %>%
  relocate(Initiation, Move, Count) 

table_vcs_3 <- first_move_byinit[,2:3] %>%
  kbl(booktabs = T, escape = F, type = "text",
  ...
  group_rows("Deceased", 1,4) %>%
  group_rows("New apartment", 5, 8) %>%
  group_rows("Out move", 9, 12) 
writeLines(table_vcs_3, "table_vcs_3.tex")

vcs_3 <- vcs[chain_step == 1] %>%
  mutate(Move = ifelse(seg_move == -1, "Desegregating", ifelse(seg_move == 1, "Segregating", "Within")),
         initiation = ifelse(initiation == "deceased", "Deceased", 
                             ifelse(initiation == "new_apartment", "New apartment", "Out move"))) %>%
  ...
  kbl(booktabs = T, escape = F, 
      caption = "Classification of first moves by type of initiation", 
      col.names = c("VC initiation", "Desegregating", "Segregating", "Within", "NA", "Total"),
      type = "text") %>% #format = "latex" or type = "text"
  kable_styling(full_width = T, latex_options = c("striped", "hold_position")) %>%
  add_header_above(c(" " = 1, "First move"=4, " "=1))
writeLines(vcs_3, "vcs_3.tex")
...
vcs_edu <- vcs_new %>%
  left_join(lisa_dt[,.(PersonLopNr, from_ar = Ar, ma_above)], by = c("PersonLopNr", "from_ar"))
...
#90% of the nas are swedes
# sum(is.na(lisa_dt$ma_above)) is 0, but
# sum(is.na(vcs_edu$ma_above)) is 13239 --> the left join above introduces NAs
# TO INVESTIGATE
...
ci_rq1_reg %>%
  ...
  scale_y_discrete(lim = rev, labels = c("Born in Sweden", "Higher education", "Sex", 
