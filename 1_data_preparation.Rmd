...
new_building_vacancies[,vacancy_year := as.character(as.integer(year_of_building_creation)-1)]
...
deso_inc <- geo_stk_dt %>%
  select(PersonalNr, Ar, BostDESO) %>%
  mutate(Ar = as.integer(Ar)) %>%
  left_join(lisa_dt %>%
              select(PersonalNr, income, Ar) %>%
              mutate(Ar = as.integer(Ar)),
            by=c("PersonalNr", "Ar"))
deso_inc <- deso_inc[,.(mean_inc = mean(income, na.rm=T)), by=c("BostDESO", "Ar")]

# Annual share of people with MA or above by DeSO
## edu_level: first digit is the level of education --> 5 or above = post-secondary education(2y or longer) and above

lisa_dt$ma_above <- ifelse(test = (lisa_dt$edu_level/100 >= 5 & lisa_dt$edu_level/100 < 7), yes = 1, no = 0)
saveRDS(lisa_dt, file = "//.../Original_dt/lisa_dt.rds")
